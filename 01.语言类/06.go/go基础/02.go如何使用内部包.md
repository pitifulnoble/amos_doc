...
其意义为 go 下载模块时首先去 https://goproxy.cn 上下载，当 goproxy.cn 上找不到该模块（返回 404 或 410 错误码）则通过 direct 方式直接到模块对应的代码仓库下载代码

GOPROXY拥有一个默认值proxy.golang.org在中国无法访问。

**GOPRIVATE**

在go1.13支持，目的是让指定的model下载不走GOPROXY(走源码拉取)，也不走GOSUM校验。直接在目标域名下载源代码。

```
GOPRIVATE=*.amos.com|*.geet.com
```
